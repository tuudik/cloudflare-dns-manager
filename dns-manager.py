...
class CloudflareDNSManager:
    ...
    def get_zone_id(self) -> Optional[str]:
        ...
        if response.status_code != 200:
            log("error", "Failed to get zone ID", zone=self.zone_name, status=response.status_code)
            return None
    ...
    def create_record(self, name: str, record_type: str, content: str, 
        ...
        if response.status_code == 200:
            log("info", "DNS record created", name=name, content=content, type=record_type, proxied=proxied)
            return True
        else:
            log("error", "Failed to create DNS record", name=name, status=response.status_code)
            return False
    
    def update_record(self, record_id: str, name: str, record_type: str, 
                     content: str, proxied: bool = False, ttl: int = 1) -> bool:
        ...
        if response.status_code == 200:
            log("info", "DNS record updated", name=name, content=content, type=record_type, proxied=proxied)
            return True
        else:
            log("error", "Failed to update DNS record", name=name, status=response.status_code)
            return False
        ...
        for record in desired_records:
            ...
            if key in existing:
                ...
                if (existing_record["content"] != content or 
                    existing_record["proxied"] != proxied or
...
def get_docker_records(docker_ip: str, global_config: Dict) -> List[Dict]:
    ...
    try:
        ...
        for container in containers:
            ...
            log("info", "Discovered Docker service", 
                container=container.name, 
                subdomain=subdomain, 
                ip=ip,
                expose=expose)
        ...
        log("error", "Failed to discover Docker records", error=str(e), error_type=type(e).__name__)
        return []
...
class DNSManagerService:
    ...
    def __init__(self, manager: CloudflareDNSManager, config_file: str, 
                 watch_docker: bool = True):
    ...
    def sync_all(self):
        ...
        with self.lock:
            try:
                ...
                docker_discovery_enabled = self.global_config.get("docker_discovery", True)
                ...
                if not all_records:
                    ...
                    log("info", "Total records to sync", 
                ...
                log("error", "Error during sync", error=str(e), error_type=type(e).__name__)
            ...
            for event in client.events(decode=True):
                ...
                if event.get('Type') == 'container':
                    ...
                    if action in ['start', 'stop', 'die', 'kill', 'rename']:
                        ...
                        log("info", "Docker event detected", 
                            action=action, 
    ...
    def start(self):
        ...
        if self.watch_docker:
            docker_thread = threading.Thread(target=self.watch_docker_events, daemon=True)
            docker_thread.start()
...
def main():
    ...
    try:
        ...
        log("error", "Failed to read API token", error=str(e), token_file=api_token_file)
        sys.exit(1)
